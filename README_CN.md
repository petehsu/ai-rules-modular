...
2. 将相关 `.md` 文件复制进去（建议改后缀为 `.mdc` 以获得更好支持）。
3. **提示：** Cursor 会自动索引这些文件。

**方法 B：`.cursorrules`（简单）**
1. 在项目根目录创建 `.cursorrules` 文件。
2. 将 `00_Core_Protocol.md` + 技术栈相关规则的内容复制进去。

### 2. Windsurf (Cascade)
1. 在项目根目录创建 `.windsurfrules` 文件。
2. 将选定规则文件的内容复制进去。
3. **注意：** Windsurf 支持自然语言，直接粘贴 Markdown 即可。

### 3. Trae IDE
1. 创建 `.trae/rules/project_rules.md`。
2. 粘贴组合后的规则内容。
